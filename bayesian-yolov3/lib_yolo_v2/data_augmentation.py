...
class DataAugmenter:
    ...
    def augment(self, img, bbox, label):
        # flip 50%
        img, bbox = tf.cond(tf.random.uniform(**self.__rand_0_1) < 0.5, true_fn=lambda: self.flip_lr(img, bbox),
                            false_fn=lambda: (img, bbox))

        # blur 5% of images
        img = tf.cond(tf.random.uniform(**self.__rand_0_1) < 0.05, true_fn=lambda: self.blur(img), false_fn=lambda: img)

        # add color augmentation (hue, brightness, ...) to 5% of images (additional to blur)
        img = tf.cond(tf.random.uniform(**self.__rand_0_1) < 0.05, true_fn=lambda: self.color_augmentations(img),
                      false_fn=lambda: img)

        # add noise augmentation (salt&pepper, gaussian noise, ...) to 5% of images (additional to blur and color augm)
        img = tf.cond(tf.random.uniform(**self.__rand_0_1) < 0.05, true_fn=lambda: self.noise_augmentations(img),
                      false_fn=lambda: img)
    ...
    def color_augmentations(self, img):
        ...
        choice = tf.random.uniform(**rand_args)
    ...
    def colored_salt_n_pepper(self, img):
        ...
        salt_mask = tf.random.uniform(shape=self.__img_size, minval=0, maxval=1)
        pepper_mask = tf.random.uniform(shape=self.__img_size, minval=0, maxval=1)
        amount = tf.random.uniform(shape=(), minval=0.0005, maxval=0.008)
        img = tf.compat.v1.where(tf.less(salt_mask, amount), self.__ones_h_w_c, img)
        img = tf.compat.v1.where(tf.less(pepper_mask, amount), self.__zeros_h_w_c, img)
    ...
    def salt_n_pepper(self, img):
        ...
        salt_mask = tf.random.uniform(shape=size, minval=0, maxval=1)
        pepper_mask = tf.random.uniform(shape=size, minval=0, maxval=1)
        amount = tf.random.uniform(shape=(), minval=0.0005, maxval=0.008)

        salt = tf.compat.v1.where(tf.less(salt_mask, amount), self.__ones_h_w, self.__zeros_h_w)
        pepper = tf.compat.v1.where(tf.less(pepper_mask, amount), -self.__ones_h_w, self.__zeros_h_w)  # note the minus sign!
    ...
    def blur(self, img):
        kernel_size = tf.random.uniform(shape=(), minval=2, maxval=4, dtype=tf.int32)  # 2 or 3
        ...
        r = tf.nn.conv2d(r, filters=kernel, strides=[1, 1, 1, 1], padding='SAME')
        g = tf.nn.conv2d(g, filters=kernel, strides=[1, 1, 1, 1], padding='SAME')
        b = tf.nn.conv2d(b, filters=kernel, strides=[1, 1, 1, 1], padding='SAME')

        img = tf.concat([r, g, b], axis=3)
        return tf.squeeze(img, axis=0)

    def additive_gaussian_noise(self, img):
        stddev = tf.random.uniform(shape=(), minval=0.001, maxval=0.05, dtype=tf.float32)

        noise = tf.random.normal(shape=self.__img_size, mean=0.0, stddev=stddev)
        img = img + noise
        return img
...
class ImageCropper:
    ...
    def random_crop_and_sometimes_rescale(self, img, bbox, label):
        ...
        img, bbox, label = tf.cond(tf.random.uniform(shape=(), minval=0.0, maxval=1.0, dtype=tf.float32) < 0.33,
    ...
    def random_crop_with_rescale(self, img, bbox, label):
        ...
        """
        with tf.compat.v1.name_scope('random_crop_and_rescale_data'):
            scale = tf.clip_by_value(tf.random.normal(shape=(), mean=0, stddev=0.5, dtype=tf.float32),
            ...
            y_mean = y_maxval / 2.
            stddev_y = y_maxval / 4.
            y_min_ind = tf.random.normal(shape=(), mean=y_mean, stddev=stddev_y, dtype=tf.float32)
            ...
            x_min_ind = tf.random.uniform(shape=(), minval=0, maxval=self.full_width - crop_width + 1,
                                          dtype=tf.int32)
            ...
            img = tf.image.resize(img, [self.crop_height, self.crop_width])

        return img, bbox, label

    def random_crop(self, img, bbox, label):
        with tf.compat.v1.name_scope('random_crop_data'):
            # prefer crops in the middle of the image in y direction (use normal dist to select y)
            ...
            # crops are uniformly distributed in x direction
            x_min_ind = tf.random.uniform(shape=(), minval=0, maxval=self.full_width - self.crop_width + 1,
                                          dtype=tf.int32)
    ...
    def center_crop(self, img, bbox, label):
        with tf.compat.v1.name_scope('center_crop_data'):
...
def crop_boxes(boxes, labels, crop_y_min, crop_x_min, crop_y_max, crop_x_max, thresh=0.25):
    with tf.compat.v1.name_scope('crop_boxes'):
        ...
        nonzero_area_indices = tf.cast(tf.reshape(tf.compat.v1.where(tf.greater(areas_clipped / areas, thresh)), [-1]), tf.int32)
