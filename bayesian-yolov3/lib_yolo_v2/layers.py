...
def flatten(*tensors):
    flattened = []
    for t in tensors:
        flattened.append(layers.flatten(t))
    return flattened
...
def aleatoric_obj_loss(det, gt):  # aleatoric classification loss attenuation of Alex Kendall, not active
    ...
    for i in range(T):
        eps = tf.random.normal(tf.shape(det['obj']), mean=0.0, stddev=1.0)
        x = det['obj'] + (obj_stddev * eps)  # sample logits
        s = tf.sigmoid(x)
        p = tf.compat.v1.where(gt['obj'] > 0.5, s, 1 - s)  # sigmoid probability of true class
        expected_value = expected_value + p
    expected_value = expected_value / float(T)

    log_loss = - tf.math.log(expected_value)  # don't forget the minus sign
    return log_loss
...
def aleatoric_cls_loss(det, gt):  # aleatoric classification loss attenuation of Alex Kendall, not active
    ...
    for i in range(T):
        eps = tf.random.normal(tf.shape(det['cls']), mean=0.0, stddev=1.0)
...
def loss_tf(det, gt, aleatoric_loss=False):
    ...
    with tf.compat.v1.name_scope('batch_size'):
    ...
    with tf.compat.v1.name_scope('loss'):
        with tf.compat.v1.name_scope('localization'):
            ...
            tf.compat.v1.losses.add_loss(loc_loss_all)

        with tf.compat.v1.name_scope('objectness'):
            # if aleatoric_loss:
            ...
            tf.compat.v1.losses.add_loss(obj_loss_all)

        with tf.compat.v1.name_scope('cls_x_entropy'):
            # if aleatoric_loss:
            ...
            tf.compat.v1.losses.add_loss(cls_loss_all)
...
def logistic_entropy(scores):
    no_obj = (1 - scores) * tf.math.log((1 - scores))
    obj = scores * tf.math.log(scores)
    entropy = -(no_obj + obj)  # Note: there is a minus sign!
    return entropy


def softmax_entropy(scores):
    entropy = - tf.reduce_sum(scores * tf.math.log(scores), axis=-1)  # Note: there is a minus sign!
    return entropy
...
def darknet_batch_norm(inputs, training, trainable):
    batch_norm_layer = tf.keras.layers.BatchNormalization(
        trainable=trainable, epsilon=1e-05
    )
    inputs = batch_norm_layer(inputs, training=training)
    return inputs


def batch_norm(inputs, training):
    # inputs = tf.compat.v1.layers.batch_normalization(inputs, training=training,
    #                                        epsilon=1e-05)
    batch_norm_layer = tf.keras.layers.BatchNormalization(
        epsilon=1e-05
    )
...
def dropout(inputs, drop_prob, standard_test_dropout=False):
    ...
    inputs = tf.compat.v1.layers.dropout(inputs, rate=drop_prob, training=training)  # we always want dropout
    return inputs
...
def upsample(inputs):
    ...
    return tf.image.resize(inputs, (2 * shape[1], 2 * shape[2]), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)  # upsample by factor 2
...
def detection_aleatoric(inputs, cls_cnt, box_cnt, weight_regularizer):
    ...
    # use linear activation!
    conv_layer = layers.Conv2D(filters, 1, strides=1, activation=None, padding='SAME',
