...
from lib_yolo_v2 import dataset_utils, data_augmentation
...
def start(model_cls, config):
    ...
    with tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(device_count={'GPU': 1})) as sess:
...
def train(sess, model, dataset, config, init_ops=None):
    ...
    with tf.compat.v1.variable_scope('optimizer'):
        with tf.control_dependencies(update_ops):
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=config['lr'])
            train_step = optimizer.minimize(model.total_loss, var_list=tf.compat.v1.trainable_variables())

    # TODO create hist summaries
    summary_op = tf.compat.v1.summary.merge_all()

    saver = tf.compat.v1.train.Saver(max_to_keep=config['ckp_max_to_keep'])
    ...
    writer_train = tf.compat.v1.summary.FileWriter(os.path.join(config['tensorboard_path'], config['run_id'], 'train'),
                                         sess.graph)
    writer_val = tf.compat.v1.summary.FileWriter(os.path.join(config['tensorboard_path'], config['run_id'], 'val'))
    try:
