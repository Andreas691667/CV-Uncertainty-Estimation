...
from lib_yolo_v2 import data_augmentation, tfdata
...
def make_parse_fn(config):
    def parse_example(example):
        ...
        features = tf.io.parse_single_example(example, features=feature_map)

        img = decode_img(features['image/encoded'], config['full_img_size'])

        # assemble bbox
        xmin = tf.sparse.to_dense(features['image/object/bbox/xmin'], default_value=0)
        ymin = tf.sparse.to_dense(features['image/object/bbox/ymin'], default_value=0)
        xmax = tf.sparse.to_dense(features['image/object/bbox/xmax'], default_value=0)
        ymax = tf.sparse.to_dense(features['image/object/bbox/ymax'], default_value=0)
        bbox = tf.stack([ymin, xmin, ymax, xmax], axis=1)  # we use the standard tf bbox format

        label = tf.cast(tf.sparse.to_dense(features['image/object/class/label'], default_value=-1),
                        dtype=tf.int32)
...
def create_dataset(config, dataset_key):
    ...
    files = tf.compat.v1.data.Dataset.list_files(info['file_pattern']).shuffle(info['num_shards'])
...
class TrainValDataset:
    def __init__(self, model_blueprint, config):
        ...
        self.__train_iterator = tf.compat.v1.data.make_one_shot_iterator(train_dataset)
        self.__val_iterator = tf.compat.v1.data.make_one_shot_iterator(val_dataset)

        # ---------------- #
        # public interface #
        # ---------------- #
        self.handle = tf.compat.v1.placeholder(tf.string, shape=[])
        self.iterator = tf.compat.v1.data.Iterator.from_string_handle(self.handle, train_dataset.output_types,
...
class ValDataset:
    def __init__(self, config, map_fns=tuple(), dataset_key='data'):
        ...
        # ---------------- #
        self.handle = tf.compat.v1.placeholder(tf.string, shape=[])
        self.iterator = tf.compat.v1.data.make_one_shot_iterator(val_dataset)
...
class TestingDataset:
    ...
    def parse_example(self, example):
        ...
        filename = tf.sparse.to_dense(features['image/filename'], default_value='')
