...
def encode_boxes(bboxes, labels, det_layers, ign_thresh):
    ...
    def loop_body(idx_, loc_x_, loc_y_, loc_w_, loc_h_, obj_, cls_, ign_):
        ...
        eps = 1e-7
        loc_x_ = tf.compat.v1.where(obj_mask, logit(tf.clip_by_value(dist_to_cell_center_x, eps, 1 - eps)), loc_x_)
        loc_y_ = tf.compat.v1.where(obj_mask, logit(tf.clip_by_value(dist_to_cell_center_y, eps, 1 - eps)), loc_y_)

        loc_w_ = tf.compat.v1.where(obj_mask, tf.math.log(tf.maximum(w[idx_] / prior_data['pw'], eps)), loc_w_)
        loc_h_ = tf.compat.v1.where(obj_mask, tf.math.log(tf.maximum(h[idx_] / prior_data['ph'], eps)), loc_h_)
        cls_ = tf.compat.v1.where(obj_mask, labels[idx_] * ones_int, cls_)
        obj_ = tf.compat.v1.where(obj_mask, ones_float, obj_)  # TODO make conditional on cls == ignore
        ign_ = tf.compat.v1.where(ign_mask, zeros, ign_)
    ...
    idx = 0
    [idx, loc_x, loc_y, loc_w, loc_h, obj, cls, ign] = tf.while_loop(cond=loop_condition, body=loop_body,
                                                                     loop_vars=[idx, loc_x, loc_y, loc_w, loc_h, obj, cls,
                                                                      ign])
...
def calc_iou(ref_bbox, prior_data):
    ...
    iou = tf.compat.v1.div(inter, union)
    return iou
