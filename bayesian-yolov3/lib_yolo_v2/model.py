...
from lib_yolo_v2 import layers, data
...
class ModelBuilder:
    def __init__(self, inputs, cls_cnt, l2_scale=0.0005):
        ...
        self.__det_layers = []
        self.__weight_regularizer = tf.keras.regularizers.l2(0.5 * (l2_scale))
    ...
    def __conv_layer(self, filters, kernel_size, strides, normalizer, variable_scope):
        ...
        with tf.compat.v1.variable_scope(None, default_name=variable_scope):
            # use_bias = True
    ...
    def make_upsample_layer(self):
        self.__current_downsample //= 2
        with tf.compat.v1.variable_scope(None, default_name='upsample'):
...
class Model:
    def __init__(self, layers, det_layers, cls_cnt, obj_idx, cls_start_idx):
        ...
        if self.det_layers[0].loc_loss is not None:
            with tf.compat.v1.name_scope('global_loss'):
                self.detection_loss = tf.compat.v1.losses.get_total_loss(add_regularization_losses=False, name='detection_loss')
                self.regularization_loss = tf.compat.v1.losses.get_regularization_loss(name='regularization_loss')
