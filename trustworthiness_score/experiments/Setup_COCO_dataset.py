...
fs = ["test", "train", "val"]
for f in fs:
   for annotation_file in os.listdir(path + 'labels/' + f):
      # Read annotation file
      data = pd.read_csv(path + 'labels/' + f+"/"+annotation_file, sep=" ", header=None)
      data.columns = ["class", "centre_x", "centre_y", "width", "height"]
      print(data)

      # Remove Labels other than for persons
      data = data.drop(data[data["class"] > 0].index)
      print(data)

      # Tweaking the annotations of a person so it cover the lower half/legs of human.
      data["centre_y"] = data["centre_y"] + (data["height"]/4)
      data["height"] = data["height"]/2
      
      print(data)

      # Remove pictures that do not have legs/persons in them
      if data.empty:
         print(True)
         os.remove(path + 'labels/' + f+"/"+annotation_file)
         img_file = annotation_file[:-3]+"jpg"
         os.remove(path + 'images/' + f+"/"+img_file)
         
      else:
         data.to_csv(path + 'labels/' + f+"/"+annotation_file, sep=" ", index=False, header=None)
